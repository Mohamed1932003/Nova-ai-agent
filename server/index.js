...
app.post("/api/chat", async (req, res) => {
  try {
    const { message, sessionId, namespace } = req.body;
    if (!message) return res.status(400).json({ error: "Message required" });

    const answer = await runAgent({ message, sessionId, namespace });
...
app.post("/api/ingest", upload.single("file"), async (req, res) => {
  try {
    ...
    const namespace = `document-${randomUUID()}`;
    await ingestData(req.file.path, namespace);
    await unlink(req.file.path).catch(() => undefined);

    return res.json({ ok: true, namespace });
  } catch (err) {
    if (req.file?.path) {
      await unlink(req.file.path).catch(() => undefined);
    }
